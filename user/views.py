...
from home.models import ContactMessage, Information, Slider, NewsLetter, FAQ, Aboutus
from news.models import Comment_news, News
...
def user_orders_product(request):
    ...
    return render(request, 'Client/user_order_product_client.html', context)
...
def user_order_product_detail_client(request, id, oid):
    ...
    return render(request, 'Client/user_order_product_detail_client.html', context)
...
def information_update(request):
    information = Information.objects.filter(status='True').order_by('-id')
    creator = Creator.objects.get(user=request.user)
...
def detail_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            details.color = form.cleaned_data.get('color')
...
def faq_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            faq = FAQ()
            faq.order_number = form.cleaned_data.get('order_number')
...
def news_comment_get(request):
    comment = Comment_news.objects.all()
    creator = Creator.objects.get(user=request.user)
...
def news_comment_edit(request, id):
    comment = Comment_news.objects.get(pk=id)
    creator = Creator.objects.get(user=request.user)
    if request.method == 'POST':
        form = EditNewsCommentForm(request.POST, instance=comment)
        if form.is_valid():
            form.save()
            messages.success(request, 'Your account has been updated')
...
def news_comment_delete(request, id):
    news_comment = Comment_news.objects.get(pk=id)
    news_comment.delete()
    return redirect('news_comment_get')


def news_comment_delete_all(request):
    news_comment = Comment_news.objects.all()
    news_comment.delete()
    return redirect('news_comment_get')
